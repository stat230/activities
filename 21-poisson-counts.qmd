...
1. Fit a Poisson regression model with `week` and `station` as the two predictor variables. Report the fitted model equation.
...
2. Interpret each exponentiated slope estimate in context. Hint: You will need to think about what level of `station` is the baseline (i.e., the one without an indicator variable).

3. Next, let's plot the fitted model. To do this, we can use effect plots. Once you have loaded the {effects} package, plug your fitted model into the blank and run the code. 

```{r eval=FALSE}
plot(allEffects(___), main = "", rescale.axis = FALSE)
```

4. Rerun the code for the effects plot, but this time set `rescale.axis = TRUE`. How does the plot change? What do you notice about the scale of the y-axis? Does this make sense?


## Conducting inference

5. Conduct an appropriate hypothesis test to determine if `station` is an important predictor. Report the hypotheses, the test statistic, the p-value, and your conclusion in context.

6. Construct an 89% Wald confidence interval for the `week` predictor.


## Model checking

7. To construct deviance residual plots we can use the same `residualPlots()` command from the {car} package. Do you see any signs of non-linearity? (Note: in order for the residual plot for `station` to show up, you should wrap specify `factor(station)` when you fit the model. I have no clue why this is needed, but apparently it is.)

8. Explore whether the effect of `week` should be linear in the model. To do this, consider whether a polynomial for `week` substantially improves the model. Report your findings.
...
9. Fit a Poisson regression model with all of the predictors included as main effects. If you found a polynomial for `week` was appropriate, then you should include that term here as well. Report the fitted model equation.

10. Create an effect plot to help you decipher your model results.