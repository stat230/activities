...
```{r}
cancer_mod <- glm(Cases ~ MedianAge, data = cancer, family = "poisson", offset = log(Person.years))
```

To begin the day, you'll fit a Poisson regression model with multiple predictor variables. To do this, you adapt the formula to add addition predictor and interaction terms just like you did for multiple linear and logistic regression models.
...
The good news is that inference for Poisson regression uses the same tools (and code!) as inference for logistic regression, it's just the interpretations that differ a bit due to the link function. So, we'll review the use of Wald's test, Wald intervals, and likelihood ratio test (LRTs).
...
The model-checking toolkit is also very similar for Poisson regression and binomial logistic regression. Specifically, we'll use deviance residuals and the deviance goodness-of-fit test.
...
```
from the {car} package (substitute your model's name in for `model`). The deviance goodness-of-fit test statistic is reported in the same place on the `summary()` as it was in logistic regression.
